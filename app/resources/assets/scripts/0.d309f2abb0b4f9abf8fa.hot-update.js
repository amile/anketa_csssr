...
/***/ function(module, exports) {
	...
	function moveSlider() {
		...
		function touchSliderThumb(eventDown) {
			var thumbBox = thumb.getBoundingClientRect();
			console.log(thumbBox);
			console.log(eventDown.pageX);
			var deltaX = eventDown.pageX - window.pageXOffset - thumbBox.left;
			var prevOnMouseMove = document.onmousemove;
			var prevOnMouseUp = document.onmouseup;
			document.onmousemove = moveSliderThumb();
			document.onmouseup = setSliderThumb();
		}
		function moveSliderThumb(eventMove) {
		...
		function setSliderThumb(eventUp) {
			clearSelection();
			...
			document.onmouseup = function (eventUp) {
				...
				if (pos < 0) {
					...
					steps.forEach(function (_, i) {
						...
						var next = i < steps.length ? steps[i + 1] : sliderBox.width;
						if (pos > curr && pos < next) {
							_stop = pos - curr < (next - curr) / 2 ? curr : next;
						}
					});
					console.log('stop ' + _stop);
					thumb.style.left = _stop - Math.floor(thumbBox.width / 2) + 'px';
				}
				document.onmousemove = prevOnMouseMove;
				document.onmouseup = prevOnMouseUp;
			};
...
//# sourceMappingURL=0.d309f2abb0b4f9abf8fa.hot-update.js.map